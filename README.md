# Ритм — iOS-клиент корпоративной платформы Rossi

Нативный SwiftUI-клиент для корпоративного портала Ритм / Rossi.
...
## Демо-режим
Кнопка «Войти в демо-версию» на экране логина (если включена) переключает
`APIClient` в локальный режим без сетевых запросов — все экраны работают
с синтетическими данными (см. `Networking/DemoMode.swift`).
